...
const library_choose_existing = {
   ...
   */
  async init() {
    const mediaArea = document.getElementById("blog-media");
    if (!mediaArea) return;
    ...
    chooseExisting.classList.add("blog-media__choose-existing");
    ...
    for (const media of allMedias.medias) {
      ...
      label.classList.add("blog-media__choose-existing-label");

      // Get extension from mime type
      const extension = media.type.split("/")[1];
      label.innerHTML = "<span class='blog-media__choose-existing-label-name'>" + media.name + "." + extension + "</span>";
      label.htmlFor = `existing_media_${media.id}`;

      // Create a div with the media preview
      const preview = document.createElement("div");
      preview.classList.add("blog-media__choose-existing-preview");

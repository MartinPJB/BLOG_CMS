...
const block_create = {
  ...
  */
  async displayForm(e) {
    ...
    const formContainer = document.querySelector("#blog-block__creation-container");
    ...
    if (e.target.dataset.json) {
      ...
      if (inputJSON.media_id) {
        await waitForElm(".blog-media__choose-existing");
      }
  ...
  */
  createFormElement(articleID, targetForm, form, blockId) {
    ...
    formHTML.id = "blog-block__creation-form";
  ...
  */
  hideForm(e) {
    document.querySelector("#step-1").classList.toggle("hidden");
    document.querySelector("#step-2").classList.toggle("hidden");
    document.querySelector("#blog-block__creation-container").innerHTML = "";
  },
  ...
  */
  assignButtons() {
    const blockButtons = document.querySelectorAll(".blog-block__creation");
    for (const button of blockButtons) {
      button.addEventListener("click", this.displayForm.bind(this));
    }

    const updateButtons = document.querySelectorAll(".blog-block__update");
    ...
    const backbutton = document.querySelector("#blog-block__creation-back");

...
final class CoreExtension extends AbstractExtension
{
    ...
    public function getFilters(): array
    {
        return [
            ...
            new TwigFilter('base64_encode', 'base64_encode'), // Ajouté
            new TwigFilter('base64_decode', 'base64_decode'), // Ajouté
    ...
    public function getFunctions(): array
    {
        return [
            ...
            /* Ajout de fonctions twig pour m'ajouter des possibilités */
            new TwigFunction('randomCoolGradient', 'twig_randomCoolGradient'), // Ajouté ligne 411
            new TwigFunction('checkImageExists', 'twig_checkImageExists'), // Ajouté ligne 439
...
function twig_randomCoolGradient($seed = 0) {
    if ($seed == 0) {
        $seed = rand(0, 10000);
    }
    ...
    // Générer les couleurs aléatoires
    $startColor = '#' . dechex(mt_rand(0x000000, 0xFFFFFF));
    $endColor = '#' . dechex(mt_rand(0x000000, 0xFFFFFF));

    $linearOrRadial = mt_rand(0, 1) ? 'linear' : 'radial';
    $svgGradient = $linearOrRadial.'Gradient';

    // Créer le code SVG pour le dégradé
    $svg = '<svg xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:svgjs="http://svgjs.dev/svgjs" viewBox="0 0 700 700" width="700" height="700">
    <defs>
    <'.$svgGradient.' x1="50%" y1="0%" x2="50%" y2="100%" id="ffflux-gradient">
        <stop stop-color="'.$startColor.'" stop-opacity="1" offset="0%"></stop>
        <stop stop-color="'.$endColor.'" stop-opacity="1" offset="100%"></stop>
    </'.$svgGradient.'>
    <filter id="ffflux-filter" x="-20%" y="-20%" width="140%" height="140%" filterUnits="objectBoundingBox" primitiveUnits="userSpaceOnUse" color-interpolation-filters="sRGB">
        <feTurbulence type="fractalNoise" baseFrequency="0.00'.mt_rand(0, 5).' 0.00'.mt_rand(0, 5).'" numOctaves="'.mt_rand(0, 2).'" seed="'.$seed.'" stitchTiles="stitch" x="0%" y="0%" width="100%" height="100%" result="turbulence"></feTurbulence>
...
function twig_checkImageExists($url) {
    $imageInfo = @getimagesize($url);
    if ($imageInfo) {
        return $url;
    }
    return twig_randomCoolGradient();
}
